...
Use `vp run dist:desktop:dmg:fork` as shorthand for `vp run dist:desktop:dmg --distribution fork`.
The fork distribution uses development branding, the `com.fuseboxhq.t3code.fork` bundle ID, the `t3code-fork` URL scheme, `~/.t3-fork` for server state, and `~/Library/Application Support/t3code-fork` for Electron state.
Its version includes the source commit.
Fork artifacts do not use the official update feed or include the official native passkey binaries.
