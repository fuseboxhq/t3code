...
| Sub-thread     | A T3 thread spawned through the `thread_*` MCP tools. Its T3 parent thread ID is separate from provider-native parent-agent lineage. |
...
| Summary                 | A model-written digest stored on a thread or project and refreshed by the summary reactor.   |
