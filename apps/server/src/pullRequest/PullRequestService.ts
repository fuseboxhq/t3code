...
export const make = Effect.gen(function* () {
  ...
  const refCacheKey = (ref: PullRequestRef) =>
    JSON.stringify([epochs.refEpoch(ref), ref.projectId, ref.repository, ref.number]);
  ...
  const statsCacheKey = (key: string) => JSON.stringify([epochs.listingsEpoch(), key]);
  ...
  const statsBatchKey = (refs: Iterable<PullRequestRef>) =>
    JSON.stringify([
      epochs.listingsEpoch(),
      [...refs]
        .map((ref) => [ref.projectId, ref.repository, ref.number, epochs.refEpoch(ref)] as const)
        .toSorted((left, right) =>
  ...
  const invalidate: PullRequestService["Service"]["invalidate"] = (input) => {
    ...
    if (reference !== undefined) {
      return Effect.sync(() => epochs.bumpRefEpoch(reference));
    }
    return Effect.sync(() => {
  ...
  const refreshAfterTurn: PullRequestService["Service"]["refreshAfterTurn"] = Effect.suspend(() => {
    const revision = epochs.bumpAllEpochs();
    return SubscriptionRef.set(pullRequestRefreshes, revision);
  });
  ...
  const runActionAndInvalidate: PullRequestService["Service"]["runAction"] = Effect.fn(
    ...
    epochs.bumpRefEpoch({ ...input, repository });
    epochs.bumpListingsEpoch();
